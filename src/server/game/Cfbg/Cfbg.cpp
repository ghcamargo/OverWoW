...
void Player::FitPlayerInTeam(bool action, Battleground* pBattleGround)
{
	...
	if (pBattleGround && action)
		SendChatMessage("%sVocê está jogando pela a %s%s em nessa %s", MSG_COLOR_WHITE, GetTeam() == ALLIANCE ? MSG_COLOR_DARKBLUE"alliance" : MSG_COLOR_RED"horde", MSG_COLOR_WHITE, pBattleGround->GetName().c_str());
}
